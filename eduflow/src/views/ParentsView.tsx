...
export function ParentsView() {
  ...
  const openLink = (parent?: any) => {
    setLinkSeed(parent || null);
    setIsLinkOpen(true);
  };
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <div className="relative">
            ...
            <input
              type="text" placeholder="Search parents..." value={searchQuery}
      ...
      <LinkModal
        isOpen={isLinkOpen}
        onClose={() => { setIsLinkOpen(false); setLinkSeed(null); }}
        seedUser={linkSeed}
        seedType={linkSeed ? 'parent' : undefined}
        onLinked={fetchParents}
      />
          ...
          <div key={parent._id} className="bg-white border border-zinc-200 rounded-[28px] p-6 hover:border-orange-200 transition-all shadow-sm">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <span className="px-2 py-0.5 bg-zinc-100 text-zinc-500 text-[9px] font-black rounded-lg uppercase tracking-widest">
                    Parent
                  </span>
                </div>
              </div>
            ...
            <div className="space-y-2 text-xs">
              ...
              <div className="flex items-center gap-2 text-zinc-500">
                ...
                {parent.studentIds?.length || 0} linked student{parent.studentIds?.length !== 1 ? 's' : ''}
              </div>
            </div>

            {canManage && (
              <button
                onClick={() => openLink(parent)}
      ...
      {filtered.length === 0 && (
        <div className="text-center py-20 text-zinc-400">
          ...
          {canManage && (
            <button
              onClick={() => setIsAddOpen(true)}
              className="mt-4 px-5 py-2.5 bg-orange-500 text-white rounded-2xl text-sm font-bold hover:bg-orange-600"
            >
              Add First Parent
            </button>
          )}
